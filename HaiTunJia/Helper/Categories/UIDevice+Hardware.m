...
- (NSString *)uniqueID {
    NSString *uid = nil;
    uid = [SFHFKeychainUtils getPasswordForUsername:@"com.changwei.www" andServiceName:@"com.changwei.www" error:nil];
    if (uid) {
        return uid;
    }
    ...
    if ([[self.systemVersion substringToIndex:1] intValue] >= 7)
    {
        ...
        for (int i = 0; i < [strarray count]; i++)
            [string appendString:[strarray objectAtIndex:i]];
    ...
    assert(uid != nil);
    [SFHFKeychainUtils storeUsername:@"com.changwei.www" andPassword:uid forServiceName:@"com.changwei.www" updateExisting:YES error:nil];
    return uid;
}
